...
accelerate launch --num_processes=4 --main_process_port=11191 \
    -m lmms_eval \
    --model llava_llada \
    --model_args pretrained=$MODEL_PATH,conv_template=llada,model_name=llava_llada \
    --tasks gqa,realworldqa,ai2d,scienceqa_img,mme,mmmu_val,textvqa_val_lite,chartqa_lite,flickr30k_test_lite,coco2017_cap_val_lite,docvqa_val_lite,infovqa_val_lite \
    --batch_size 1 \
    --gen_kwargs alg=topk_margin,prefix_lm=True,max_new_tokens=256 \
    --log_samples \
