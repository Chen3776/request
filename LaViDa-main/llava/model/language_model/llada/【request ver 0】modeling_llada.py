...
class LLaDABlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.sparse_attn_top_k = getattr(config, "sparse_attn_top_k", 512)
        self.sparse_attn_interval = getattr(config, "sparse_attn_interval", 4)
    ...
    def _sparse_attention_logic(
        ...
        --- 已修正 ---
        包含完整稀疏注意力机制的函数。
        用手动矩阵乘法替代 F.scaled_dot_product_attention 以处理5维稀疏张量。
        """
        # 1. GQA 支持: 确保 Q 和 K/V 的头数量匹配
        assert k.size(1) == v.size(1)
        num_kv_heads = k.size(1)
        num_q_heads = q.size(1)
        if num_q_heads != num_kv_heads:
        ...
        B, nH, T_q, hs = q.shape
        T_k = k.shape[2]
        k_val = min(self.sparse_attn_top_k, T_k)
        
        # 2. 判断是否是 reference step
        is_reference_step = (self.decode_step_counter == 0) or (self.decode_step_counter % self.sparse_attn_interval == 0)

        if is_reference_step:
            attn_scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(hs)
            _, self.top_k_indices = torch.topk(attn_scores, k_val, dim=-1, sorted=False)
        
        if self.top_k_indices is None:
            # 如果由于某种原因索引不存在，则回退到密集注意力以保证程序健壮性
            return F.scaled_dot_product_attention(q, k, v, dropout_p=dropout_p, is_causal=False)

        # 3. 使用缓存的索引进行 gather
        # k_sparse/v_sparse 的形状为 (B, nH, T_q, k_val, hs)
        indices_expanded = self.top_k_indices.unsqueeze(-1).expand(-1, -1, -1, -1, hs)
        k_broadcasted = k.unsqueeze(2).expand(-1, -1, T_q, -1, -1)
        v_broadcasted = v.unsqueeze(2).expand(-1, -1, T_q, -1, -1)
        k_sparse = torch.gather(k_broadcasted, 3, indices_expanded)
        v_sparse = torch.gather(v_broadcasted, 3, indices_expanded)
        
        # --- 修正点: 手动实现 scaled-dot-product-attention ---
        # 4. 计算稀疏注意力
        
        # q 的形状是 (B, nH, T_q, hs), 我们需要 unsqueeze 成 (B, nH, T_q, 1, hs) 以便和 k_sparse 做矩阵乘法
        q_for_scores = q.unsqueeze(3)
        
        # (B, nH, T_q, 1, hs) @ (B, nH, T_q, hs, k_val) -> (B, nH, T_q, 1, k_val)
        scores = torch.matmul(q_for_scores, k_sparse.transpose(-2, -1))
        scores = scores.squeeze(3) # -> (B, nH, T_q, k_val)
        ...
        # 将权重应用到 v_sparse
        # attn_weights unsqueeze 成 (B, nH, T_q, 1, k_val)
        # (B, nH, T_q, 1, k_val) @ (B, nH, T_q, k_val, hs) -> (B, nH, T_q, 1, hs)
        output = torch.matmul(attn_weights.unsqueeze(3), v_sparse)
        output = output.squeeze(3) # -> (B, nH, T_q, hs)
    ...
    def attention(
        ...
        # --- 修改: attention 函数作为总调度中心 ---
        # Get the attention scores.
        # shape: (B, nh, T, hs)
        dropout_p = 0.0 if not self.training else self.config.attention_dropout
        
        # 调度逻辑: 根据条件选择不同的注意力实现
        if not self.training and self.use_sparse_attention:
            # 模式1: 解码时使用稀疏注意力
            att = self._sparse_attention_logic(q, k, v, dropout_p=dropout_p)
        elif block_mask is not None:
            # 模式2: 使用 flex attention
            att = self._flex_attention(q, k, v, dropout_p=dropout_p, block_mask=block_mask)
        else:
            # 模式3: 默认使用标准的密集注意力
            att = self._scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=dropout_p, is_causal=False)
        # --- 修改结束 ---
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        if hasattr(self.model, "use_sparse_attention") and self.model.use_sparse_attention:
            is_new_generation = past_key_values is None
        ...
        if use_cache is None:
            ...
            if hasattr(self.model.transformer, "blocks"):
                ...
                for group in self.model.transformer.block_groups:
                    blocks.extend(group)
            
            for block in blocks:
                if is_new_generation:
                    block.reset_generation_state()
                if not self.training:
                    block.decode_step_counter += 1
