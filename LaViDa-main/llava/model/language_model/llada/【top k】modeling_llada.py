...
class LLaDABlock(nn.Module):
    ...
    # 文件: modeling_llada.py
    # 类: LLaDABlock

    # 请用这个完整函数替换旧的 _stateful_sparse_attention 方法
    def _stateful_sparse_attention(
        ...
        """
        实现一个有状态的、动态更新的稀疏注意力机制。(已修复T_q维度不匹配的BUG)
        """
        import math
        ...
        if past_length == 0 or (past_length > 0 and past_length % 4 == 0):
            ...
            # 因为 unique 后长度可能不一，需要pad到一致
            max_len = max(len(t) for t in unique_indices_list)
            padded_indices = torch.stack([
                torch.nn.functional.pad(t, (0, max_len - len(t)), 'constant', T_k) # 使用一个不会被用到的值 T_k 作为 padding
                for t in unique_indices_list
            ])
            top_k_indices = padded_indices.view(original_shape[0], original_shape[1], original_shape[2], -1)
            # 移除 padding 用的值
            top_k_indices = top_k_indices[top_k_indices != T_k].view(original_shape[0], original_shape[1], original_shape[2], -1)
    
            # 根据合并后的索引，从 K 矩阵中 gather 出需要的键
            k_indices_expanded = top_k_indices.unsqueeze(-1).expand(-1, -1, -1, -1, hs)
            k_expanded = k.unsqueeze(2).expand(-1, -1, T_q, -1, -1)
            k_gathered = torch.gather(k_expanded, 3, k_indices_expanded)
            
            top_k_scores = torch.matmul(q.unsqueeze(3), k_gathered.transpose(-2, -1)).squeeze(3) / math.sqrt(hs)
    
        # --- 后续计算（刷新和重用步骤通用）---
        sparse_attn_weights = torch.nn.functional.softmax(top_k_scores, dim=-1)
        if dropout_p > 0.0:
            sparse_attn_weights = torch.nn.functional.dropout(sparse_attn_weights, p=dropout_p)
        
        # 根据相同的索引从 V 矩阵中 gather 出值
        v_indices_expanded = top_k_indices.unsqueeze(-1).expand(-1, -1, -1, -1, hs)
        v_expanded = v.unsqueeze(2).expand(-1, -1, T_q, -1, -1)
        v_gathered = torch.gather(v_expanded, 3, v_indices_expanded)
    
        output = torch.matmul(sparse_attn_weights.unsqueeze(3), v_gathered).squeeze(3)
    ...
    # 请将这个新方法添加到 LLaDABlock 类的定义内部
    # 例如，可以放在 _scaled_dot_product_attention 方法的上面或下面
    def _top_k_sparse_attention(
        ...
        一个实验性的稀疏注意力实现：
        1. 计算完整的 (Q, K^T) 注意力分数矩阵。
        2. 对每个 Query，找到分数最高的 Top-K 个 Key 的索引。
        3. 根据索引，只与这 Top-K 个 Key 和 Value 进行注意力计算。
        """
        import math
        ...
        # --- 1. 计算完整的注意力分数 (不经过 softmax) ---
        # 这是计算开销最大的一步
        # ahape: (B, nh, T_q, T_k)
        full_scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(hs)

        # --- 2. 找到每个 Query 对应的 Top-K 个 Key ---
        # 我们在最后一个维度 (T_k) 上寻找 top_k
        # top_k_scores 的形状: (B, nh, T_q, top_k)
        # top_k_indices 的形状: (B, nh, T_q, top_k)
        top_k_scores, top_k_indices = torch.topk(full_scores, k=top_k, dim=-1)

        # --- 3. 根据 Top-K 索引筛选 Key 和 Value ---
        # `torch.gather` 是实现这个筛选的关键
        # k 和 v 的原始形状: (B, nh, T_k, hs)
        # top_k_indices 的形状: (B, nh, T_q, top_k)

        # 为了使用 gather, 我们需要让 index 和 source 的维度匹配
        # 我们需要为 k 和 v 扩展 hs 维度
        # ahape: (B, nh, T_q, top_k, hs)
        top_k_indices_expanded = top_k_indices.unsqueeze(-1).expand(-1, -1, -1, -1, hs)

        # 从 k 和 v 中收集 (gather) 对应的向量
        # ahape: (B, nh, T_k, hs) -> (B, nh, 1, T_k, hs) -> (B, nh, T_q, T_k, hs)
        # 这样做是为了让每个 query 都能从完整的 key 序列中筛选
        k_expanded = k.unsqueeze(2).expand(-1, -1, T_q, -1, -1)
        v_expanded = v.unsqueeze(2).expand(-1, -1, T_q, -1, -1)

        # ahape: (B, nh, T_q, top_k, hs)
        top_k_k = torch.gather(k_expanded, 3, top_k_indices_expanded)
        top_k_v = torch.gather(v_expanded, 3, top_k_indices_expanded)

        # --- 4. 在筛选后的 K, V 上计算最终注意力 ---
        # 注意：这里的 scaled_dot_product_attention 不能直接用，因为它期望 Q, K, V 的序列长度匹配
        # 我们直接使用已经计算好的 top_k_scores

        # shape: (B, nh, T_q, top_k)
        sparse_attn_weights = torch.nn.functional.softmax(top_k_scores, dim=-1)
        if dropout_p > 0.0:
            sparse_attn_weights = torch.nn.functional.dropout(sparse_attn_weights, p=dropout_p)

        # ahape: (B, nh, T_q, hs)
        # (B, nh, T_q, top_k) @ (B, nh, T_q, top_k, hs) -> (B, nh, T_q, 1, hs) -> (B, nh, T_q, hs)
        output = torch.matmul(sparse_attn_weights.unsqueeze(3), top_k_v).squeeze(3)
    ...
    def attention(
        ...
        if block_mask is not None:
            ...
            # k_val = current_seq_len // 2
            
            # att = self._top_k_sparse_attention(
            #     q,
            #     k,
            #     v,
            #     top_k=k_val,
            ...
            # att = self._stateful_sparse_attention(
            #     q,
            #     k,
            #     v,
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if (
                    ...
                    # )
                    x, cache = self._activation_checkpoint_fn(
                        block, x, attention_bias, layer_past, use_cache,block_mask, past_length=past_length
                    )
                else:
                    # shape: (batch_size, seq_len, d_model)
                    x, cache = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache,block_mask=block_mask,past_length=past_length)
                if attn_key_values is not None:
                    assert cache is not None
                    attn_key_values.append(cache)
        else:
