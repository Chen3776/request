...
def causal_test(BS, HEAD, SEQLEN, DIM, causal):
    ...
    # time_tri = time.time()
    # tri_out = flash_attn_triton(q, k, v, causal=causal, sm_scale=sm_scale)
    # time_tri = time.time() - time_tri

    # 1. 统一预热 (Warm-up for ALL kernels)
    # 预热PyTorch的cuBLAS/cuDNN核函数和Triton的JIT编译
    print("Warming up PyTorch implementation...")
    _ = ref_attn(q, k, v, causal=causal, sm_scale=sm_scale)
    print("Warming up Triton implementation...")
    _ = flash_attn_triton(q, k, v, causal=causal, sm_scale=sm_scale)
    
    # 等待所有预热工作在GPU上完成，确保GPU空闲
    ...
    end_event.record()
    torch.cuda.synchronize()
    time_ref = start_event.elapsed_time(end_event) # 直接给出毫秒    
    ...
    end_event.record()
    torch.cuda.synchronize()
    time_tri = start_event.elapsed_time(end_event) # 直接给出毫秒

    # compare
    assert torch.allclose(ref_out, tri_out, atol=1e-2, rtol=0)
    print("causal = {}, ref time: {:.4f} ms, tri time: {:.4f} ms.".format(causal, time_ref, time_tri))
